...
if __name__ == '__main__':
    # In input parameters :
    # result folder
    # The result folder contains several folders containing .csv files
    folder_result = sys.argv[1]
    folder_list = []
    ...
    for i in range(len(folder_list)):
        ...
        if len(file_name_parts) >= 4:
            ...
            # On ne garde que les lignes intéressantes
    ...
    for root, _, files in os.walk("./" + folder_result + "/energy/calltrees", topdown=False):
        for name in files:
    ...
    for root, _, files in os.walk("./" + folder_result + "/power/methods", topdown=False):
        for name in files:
    ...
    for root, _, files in os.walk("./" + folder_result + "/power/calltrees", topdown=False):
        for name in files:
    ...
    # to_csv_df.to_csv("./" + folder_result + nom_fichier, index=False)

    # Exporter les données en CSV
    pd_energy.to_csv("./" + folder_result +
                     "/energy_methods_summary.csv", index=False)
    pd_power.to_csv("./" + folder_result +
                    "/power_methods_summary.csv", index=False)

    # Read energy_summary.csv and power_summary.csv into dataframes
    energy_df = pd.read_csv('./' + folder_result +
                            '/energy_methods_summary.csv')
    power_df = pd.read_csv('./' + folder_result + '/power_methods_summary.csv')
    ...
    pd_energy_1.to_csv("./" + folder_result +
                       "/energy_calltrees_summary.csv", index=False, sep=';')
    pd_power_1.to_csv("./" + folder_result +
                      "/power_calltrees_summary.csv", index=False, sep=';')

    # Read energy_summary.csv and power_summary.csv into dataframes with the same delimiter
    energy_df_1 = pd.read_csv(
        './' + folder_result + '/energy_calltrees_summary.csv', sep=';')
    power_df_1 = pd.read_csv('./' + folder_result +
                             '/power_calltrees_summary.csv', sep=';')

    # Concatenate energy_df and power_df vertically
    combined_df = pd.concat([energy_df_1, power_df_1], axis=0)
    # Write combined_df to joularJX.csv with the same delimiter
    combined_df.to_csv('jx_results/joularJX_calltrees.csv',
                       index=False, sep=';')
