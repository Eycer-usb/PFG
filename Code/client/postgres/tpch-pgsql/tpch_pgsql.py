...
def main(phase, host, port, user, password, database,
         ...
         scale, num_streams, verbose, read_only, decompress):
    ...
    :param decompress: True if data files are stored in compressed format
    ...
    if phase == "prepare":
        ...
        if decompress:
            if load.decompress_data(query_root, host, port, database, user, password, PREP_QUERY_DIR):
                print("could not decompress data files")
                exit(1)
            print("done decompressing data files")
        else:
            print("compressing data files")
            if load.compress_data(query_root, host, port, database, user, password, PREP_QUERY_DIR):
                print("could not compress data files")
                exit(1)
            print("done compressing data files")
            # if load.force_compression(query_root, host, port, database, user, password, PREP_QUERY_DIR):
            #     print("could not force compression")
            #     exit(1)
            # print("done forcing compression")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-z", "--decompress", action="store_true",
                        help="Decompress data files before loading them into the database")
    ...
    # main
    main(phase, host, port, user, password, database, dbgen_dir,
         data_dir, query_root, scale, num_streams, verbose, read_only, decompress)
