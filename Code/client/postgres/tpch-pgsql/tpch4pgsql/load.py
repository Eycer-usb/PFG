...
def create_schema(query_root, host, port, db_name, user, password, prep_query_dir):
    ...
    try:
        conn = pgdb.PGDB(host, port, db_name, user, password)
        try:
            conn.executeQueryFromFile(os.path.join(
                query_root, prep_query_dir, "create_tbl.sql"))
        except Exception as e:
...
def decompress_data(query_root, host, port, db_name, user, password, prep_query_dir):
    """Decompresses the tables.
    ...
    try:
        ...
        try:
            conn.executeQueryFromFile(os.path.join(
                query_root, prep_query_dir, "decompress.sql"))
        except Exception as e:
            print("unable to run decompress tables. %s" % e)
            return 1
        conn.commit()
        conn.close()
    except Exception as e:
...
def compress_data(query_root, host, port, db_name, user, password, prep_query_dir):
    """Compresses the tables.
    ...
    try:
        ...
        try:
            conn.executeQueryFromFile(os.path.join(
                query_root, prep_query_dir, "compress.sql"))
        except Exception as e:
            print("unable to run compress tables. %s" % e)
            return 1
        conn.commit()
        conn.close()
    except Exception as e:
...
def force_compression(query_root, host, port, db_name, user, password, prep_query_dir):
    """Compresses the tables.
    ...
    try:
        ...
        try:
            conn.executeQueryFromFile(os.path.join(
                query_root, prep_query_dir, "force_compress.sql"))
        except Exception as e:
            print("unable to run force compress tables. %s" % e)
            return 1
        conn.commit()
        conn.close()
    except Exception as e:
        ...
        try:
            for table in tables:
                filepath = os.path.join(
                    data_dir, load_dir, table.lower() + ".tbl.csv")
...
def index_tables(query_root, host, port, db_name, user, password, prep_query_dir):
    ...
    try:
        ...
        try:
            conn.executeQueryFromFile(os.path.join(
                query_root, prep_query_dir, "create_idx.sql"))
            conn.commit()
        except Exception as e:
